...
class Supervision_Train(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        if self.config.weights_name == 'UANet_VGG' or self.config.weights_name == 'UANet_Res50'\
        or self.config.weights_name == 'UANet_Res250':
            seg_pre = self.net(x)[4]
        else:
            seg_pre = self.net(x)
        return seg_pre
    ...
    def training_step(self, batch, batch_idx):
        ...
        or self.config.weights_name == 'UANet_Res250':
            ...
            loss = self.loss(predictions[0], mask) + self.loss(predictions[1], mask) + self.loss(predictions[2], mask) + self.loss(predictions[3], mask) + self.loss(predictions[4], mask)
            prediction = predictions[4]


        elif self.config.weights_name == 'UANet_PVT':
            predictions = self.net(img)
    ...
    def training_epoch_end(self, outputs):
        ...
        elif 'vaihingen' in self.config.log_name:
            mIoU = np.nanmean(self.metrics_train.Intersection_over_Union()[:-1])
            F1 = np.nanmean(self.metrics_train.F1()[:-1])
        elif 'mmflood' in self.config.log_name:
    ...
    def validation_step(self, batch, batch_idx):
        img, mask = batch['img'], batch['gt_semantic_seg']
        if self.config.weights_name == "SANet_large_edge_all":
